...
class MatchPyramid(BasicModel):
    ...
    def build(self):
        ...
        query_pos = Input(name='query_pos', shape=(self.config['text1_maxlen'],))
        show_layer_info('Input', query_pos)

        doc = Input(name='doc', shape=(self.config['text2_maxlen'],))
        show_layer_info('Input', doc)
        doc_pos = Input(name='doc_pos', shape=(self.config['text2_maxlen'],))
        ...
        # q_merged = Reshape((self.config['text1_maxlen'],self.config['pos_embed_size']+self.config['embed_size']))(q_merged)
        # d_merged = Reshape((self.config['text2_maxlen'],self.config['pos_embed_size']+self.config['embed_size']))(d_merged)

        cross = Dot(axes=[2, 2], normalize=False)([q_merged, d_merged])
        ...
        model = Model(inputs=[query, query_pos, doc, doc_pos, dpool_index], outputs=out_)
        return model
=======
# -*- coding=utf-8 -*-
from __future__ import print_function
...
class MatchPyramid(BasicModel):
    ...
    def build(self):
        ...
        model = Model(inputs=[query, query_pos, doc, doc_pos, dpool_index], outputs=out_)
        return model
>>>>>>> d8d32c9c2468e18329d87f4e902e474bfa9fb3de
